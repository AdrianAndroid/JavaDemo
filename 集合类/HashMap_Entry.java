    ...
    implements Map<K,V>, Cloneable, Serializable
{
    ...
    // selected values.
    final float loadFactor = DEFAULT_LOAD_FACTOR;//负载因子，代表了table的填充度有多少，默认是0.75
    ...
    void resize(int newCapacity) {
        ...
        if (oldCapacity == MAXIMUM_CAPACITY) { // 不能超过MAX_VALUE
    ...
    void addEntry(int hash, K key, V value, int bucketIndex) {
        if ((size >= threshold) && (null != table[bucketIndex])) { // true 则需要扩容
    ...
    void createEntry(int hash, K key, V value, int bucketIndex) {
        HashMapEntry<K,V> e = table[bucketIndex]; // 1. 找出里面的值
        table[bucketIndex] = new HashMapEntry<>(hash, key, value, e); /// 2。创建一个HashMapEntry，并赋值，头插法
        size++;
    }
